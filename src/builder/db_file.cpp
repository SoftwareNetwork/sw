...
static void load(const path &fn, ConcurrentHashMap<path, FileData> &files)
{
...
static void load(const path &fn, ConcurrentHashMap<path, FileRecord> &files, std::unordered_map<int64_t, std::unordered_set<int64_t>> &deps)
{
    ...
    while (!feof(fp))
    {
        ...
        auto kv = files.insert(h);
        kv.first->data = getFileDataStorage().registerFile(p);
...
void FileDb::load(ConcurrentHashMap<path, FileData> &files) const
{
    sw::load(fn.u8string() + ".files", files);
    sw::load(getFilesLogFileName(), files);
}

void FileDb::save(ConcurrentHashMap<path, FileData> &files) const
{
    const auto f = fn.u8string() + ".files";

    // first, we load current copy of files
    ConcurrentHashMap<path, FileData> old;
    load(old);
...
void FileDb::load(const String &config, ConcurrentHashMap<path, FileRecord> &files) const
{
    ...
    sw::load(fn.u8string() + "." + config + ".files", files, deps);
    ...
    for (auto &[k, v] : deps)
    {
        for (auto &h2 : v)
        {
            ...
            auto k2 = &files[h2];
            if (k2)
                files[k].implicit_dependencies.insert({ k2->data->file, k2 });
        }
    }
}
...
{
    const auto f = fn.u8string() + "." + config + ".files";
    ...
    {
        auto &f = *i.getValue();
        auto[ptr, inserted] = files.insert(f.data->file, f);
        if (!inserted && *ptr < f)
            *ptr = f;
    }
...
void FileDb::write(std::vector<uint8_t> &v, const FileData &f) const
{
    v.clear();
...
void FileDb::write(std::vector<uint8_t> &v, const FileRecord &f) const
{
    ...
    write_int(v, std::hash<path>()(f.data->file));
    write_str(v, normalize_path(f.data->file));

    auto n = f.implicit_dependencies.size();
    write_int(v, n);

    for (auto &[f, d] : f.implicit_dependencies)
        write_int(v, std::hash<path>()(d->data->file));
}
