...
struct SW_CORE_API IDriver
{
    ...
    using package_loader_ptr = std::shared_ptr<package_loader>;
    virtual package_loader_ptr load_package(const Package &) = 0;
    virtual std::vector<package_loader_ptr> load_packages(const path &) = 0;
};
