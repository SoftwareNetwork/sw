...
PackageStore::PackageStore(ISwContext &swctx)
    : swctx(swctx)
{
}
...
void PackageStore::loadLockFile(const path &fn)
{
    ...
    const auto &overridden = swctx.getLocalStorage().getServiceDatabase().getOverriddenPackages();
...
//void Resolver::resolve(const UnresolvedPackages &dependencies)
//{
    ...
    for (auto &d : dependencies)
    {
        ...
        if (!clean_resolve)
        {
            ...
            auto i = swctx.store->resolved_packages.find(d);
            if (i != swctx.store->resolved_packages.end())
            {
    ...
    swctx.store->resolved_packages.insert(resolved_packages.begin(), resolved_packages.end());
    swctx.store->download_dependencies_.insert(download_dependencies_.begin(), download_dependencies_.end());*/
//}
